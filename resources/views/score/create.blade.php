            ...
            <div class="bg-white shadow-lg rounded-lg my-6 p-6">
                <form action="{{ route('score.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="flex items-center hidden">
                        <input type="checkbox" name="isAdult" id="isAdult" value="1" class="h-4 w-4 text-indigo-600 border-gray-300 rounded">
                        <label for="isAdult" class="ml-2 block text-sm font-medium text-gray-700">Is volwassene (18+)</label>
                        @error('isAdult')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
